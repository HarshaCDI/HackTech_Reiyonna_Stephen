...
def preprocess_data(src_root, dst_root):
    ...
    dst_img_dir_train = os.path.join(dst_root, 'images', 'train')
            ...
            for img_file in os.listdir(seq_path):
                if img_file.endswith('.jpg'):
                    ...
                    if os.path.isfile(img_path):
                        dst_seq_dir = os.path.join(dst_img_dir_train, seq_dir)
...
def generate_dot_train_file(data_root, rel_path, out_root):
    if not (os.path.isdir(data_root) and os.path.isdir(out_root)):
        print('[Err]: invalid root')
        return

    out_f_path = os.path.join(out_root, 'detrac.train')
    count = 0
...
def clean_train_set_mismatch(img_root, label_root):
...
if __name__ == '__main__':
    preprocess_data(
        src_root='../data/Insight-MVT_Annotation_Train',
        dst_root='../data/DETRAC'
    )

    generate_labels(
        xml_root='../data/DETRAC-Train-Annotations-XML',
        img_root='../data/DETRAC/images/test',
        label_root='../data/MOT/DETRAC/labels/train',
        viz_root='../data/viz_result'
    )

    clean_train_set_mismatch(
        img_root='../data/DETRAC/images/train',
        label_root='../data/DETRAC/labels/train'
    )

    generate_dot_train_file(
        data_root='../data/MOT',
